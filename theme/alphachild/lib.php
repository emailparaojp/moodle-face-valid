 ...
 * @return string
 */
function theme_alphachild_get_setting($setting, $format = false) {
    $theme = theme_config::load('alphachild');
 ...
 */
function theme_alphachild_css_tree_post_processor($tree, $theme) {
    debugging('theme_alphachild_css_tree_post_processor() is deprecated. Required' .
        'prefixes for Bootstrap are now in theme/alphachild/scss/moodle/prefixes.scss');
    $prefixer = new theme_alphachild\autoprefixer($tree);
    $prefixer->prefix();
}
 ...
 * @return array[]
 */
function theme_alphachild_user_preferences(): array {
    return [
        'drawer-open-block' => [
            'type' => PARAM_BOOL,
 ...
 * @return string
 */
function theme_alphachild_get_extra_scss($theme) {
    $content = '';

    // Sets the login background image.
    // Check login layout, only layout #1 has background image.
    $loginlayout = theme_alphachild_get_setting('setloginlayout');
 ...
 * @return bool
 */
function theme_alphachild_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        ...
        if ($filearea === 'hvp') {
            return theme_alphachild_serve_hvp_css($args[1], $theme);
        }
        if ($filearea === 'favicon') {
 ...
 */
function theme_alphachild_setting_file_url($setting, $filearea, $theme) {
    global $CFG;

    $component = 'theme_alphachild';
    $itemid = 0;
 ...
 * @return string
 */
function theme_alphachild_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_alphachild', 'preset', 0, '/', $filename))) {
 ...
 */
function theme_alphachild_get_precompiled_css() {
    global $CFG;
 ...
 * @return array
 */
function theme_alphachild_get_pre_scss($theme) {
    global $CFG;
 ...
 * @return string.
 */
function theme_alphachild_get_course_guest_access_hint($courseid) {
    global $CFG;
 ...
 * @return string.
 */
function theme_alphachild_get_course_information_banners() {
    ...
    if (get_config('theme_alphachild', 'showhintcoursehidden') == 1
            && $allowtoshowhint == true
            && $PAGE->has_set_url()
        ...
        $html .= $OUTPUT->render_from_template('theme_alphachild/course-hint-hidden', $templatecontext);
    }
    ...
    // intended.
    if (get_config('theme_alphachild', 'showhintcourseguestaccess') == 1
        ...
        // Render template and add it to HTML code.
        $html .= $OUTPUT->render_from_template('theme_alphachild/course-hint-guestaccess', $templatecontext);
    }
    ...
    if (get_config('theme_alphachild', 'showhintcourseselfenrol') == 1
            && $allowtoshowhint == true
            && $PAGE->has_set_url()
            && $PAGE->url->compare(new moodle_url('/course/view.php'), URL_MATCH_BASE)
            && $COURSE->visible == true) {
        ...
        if (!empty($selfenrolinstances) &&
            ...
            if ($selfenrolmentpossiblecurrently == true) {
                $templatecontext['selfenrolhintstart'] = get_string('showhintcourseselfenrolstartcurrently', 'theme_alphachild');
            } else if ($selfenrolmentpossiblefuture == true) {
                $templatecontext['selfenrolhintstart'] = get_string('showhintcourseselfenrolstartfuture', 'theme_alphachild');
            }
            ...
            foreach ($selfenrolinstances as $selfenrolinstanceid => $selfenrolinstanceobject) {
                ...
                if ($selfenrolinstanceobject->unrestrictedness == 'unlimited') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolunlimited', 'theme_alphachild',
                            array('name' => $selfenrolinstanceobject->name));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'until') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenroluntil', 'theme_alphachild',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'from') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolfrom', 'theme_alphachild',
                            array('name' => $selfenrolinstanceobject->name,
                                    'from' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'since') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolsince', 'theme_alphachild',
                            array('name' => $selfenrolinstanceobject->name,
                                    'since' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'fromuntil') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolfromuntil', 'theme_alphachild',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate),
                                    'from' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'sinceuntil') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolsinceuntil', 'theme_alphachild',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate),
                                    'since' => userdate($selfenrolinstanceobject->startdate)));
                }
            ...
            // Render template and add it to HTML code.
            $html .= $OUTPUT->render_from_template('theme_alphachild/course-hint-selfenrol', $templatecontext);
        }
    }
    ...
    if (get_config('theme_alphachild', 'showswitchedroleincourse') == 1
            && is_role_switched($COURSE->id) ) {
        ...
        // Render template and add it to HTML code.
        $html .= $OUTPUT->render_from_template('theme_alphachild/course-hint-switchedrole', $templatecontext);
    }
 ...
 * @throws dml_exception
 */
function theme_alphachild_serve_hvp_css($filename, $theme) {
    global $CFG, $PAGE;
    ...
    $settings = new \theme_alphachild\util\theme_settings();
    $content = $settings->hvpcss;

    $md5content = md5($content);
    $md5stored = get_config('theme_alphachild', 'hvpccssmd5');
