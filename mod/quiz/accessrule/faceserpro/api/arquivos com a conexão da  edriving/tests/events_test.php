...
class dashboard_events_testcase extends advanced_testcase {
    ...
    public function test_dashboard_reset() {
        ...
        $this->assertEquals($user->id, $event->userid);
        $this->assertEquals(quiz_PAGE_PRIVATE, $event->other['private']);
        $this->assertEquals('quiz-index', $event->other['pagetype']);
        $this->assertDebuggingNotCalled();

        // Reset the dashboard with private parameter is set to quiz_PAGE_PUBLIC and pagetype set to 'user-profile'.
        $sink = $this->redirectEvents();
        quiz_reset_page($user->id, quiz_PAGE_PUBLIC, 'user-profile');

        // Trigger and capture the event.
        $events = $sink->get_events();
        $event = reset($events);
        $this->assertEquals(quiz_PAGE_PUBLIC, $event->other['private']);
    ...
    public function test_dashboards_reset() {
        ...
        // Reset the dashboards with private parameter is set to quiz_PAGE_PUBLIC and pagetype set to 'user-profile'.
        $sink = $this->redirectEvents();
        quiz_reset_page_for_all_users(quiz_PAGE_PUBLIC, 'user-profile');
