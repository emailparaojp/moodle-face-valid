...
/**
 * Privacy for the quizaccess_faceserpro plugin.
 * @package    quizaccess_faceserpro
 * @subpackage Quiz_moodle
 * @copyright  RCN 2024
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace quizaccess_faceserpro\privacy;
...
class provider implements
    ...
    /**
     * Provides meta data that is stored about a user with quizaccess_faceserpro
     *
     * @param collection $collection A collection of meta data items to be added to.
     * @return  collection Returns the collection of metadata.
     */
    public static function get_metadata(collection $collection): collection {
        $quizaccessfaceserprologs = [
        ...
        $collection->add_database_table(
            'quizaccess_faceserpro_logs',
            $quizaccessfaceserprologs,
            'privacy:metadata:quizaccess_faceserpro_logs'
        );
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        // Context in Quizaccess faceserpro logs.
        $sql = "SELECT DISTINCT c.id
                  FROM {quizaccess_faceserpro_logs} qpl
                  JOIN {context} c ON c.instanceid = qpl.quizid AND c.contextlevel = :context
                  WHERE qpl.userid = :userid
              GROUP BY c.id";
        $contextlist = new contextlist();
        $contextlist->add_from_sql($sql, $params);
        $fileparams = ['component' => 'quizaccess_faceserpro', 'userid' => $userid];
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $fileparams = ['component' => 'quizaccess_faceserpro', 'contextid' => $context->id];
            ...
            if ($context->contextlevel === CONTEXT_MODULE && $context->instanceid) {
             ...
             ORDER BY qpl.id ASC";
                ...
                foreach ($qaplogs as $qaplog) {
                    ...
                    $index++;
                    $subcontext = [
                        get_string('quizaccess_faceserpro', 'quizaccess_faceserpro'),
                        'faceserpro_logs',
                        $index
                    ];
                    $data = (object)[
                    ...
                    if (!empty($webcamepiclast)) {
                        ...
                        writer::with_context($context)
                            ->export_area_files([get_string('privacy:core_files', 'quizaccess_faceserpro')],
                                'quizaccess_faceserpro', 'picture', $userfiles->itemid
    ...
    public static function delete_data_for_all_users_in_context(context $context) {
        ...
        if ($context->contextlevel === CONTEXT_MODULE) {
            ...
            $DB->set_field_select('quizaccess_faceserpro_logs', 'userid', 0, "quizid = :quizid", $params);
        }
        // Delete all of the webcam images for this user.
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'quizaccess_faceserpro', 'picture');
    }
    /**
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        if ($context->contextlevel !== CONTEXT_MODULE) {
            ...
            $DB->set_field_select('quizaccess_faceserpro_logs', 'userid', 0, "userid {$insql}", $inparams);
            ...
            foreach ($usersfile as $file):
                $fs->delete_area_files($context->id, 'quizaccess_faceserpro', 'picture', $file->id);
            endforeach;
        }
    }
    /**
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        $params['userid'] = $contextlist->get_user()->id;
        $DB->set_field_select('quizaccess_faceserpro_logs', 'userid', 0, "userid = :userid", $params);
        foreach ($contextlist as $context) {
            // Delete user file (webcam images).
            $userfiles = $DB->get_records('files', $params);
            $fs = get_file_storage();
            foreach ($userfiles as $file):
                $fs->delete_area_files($context->id, 'quizaccess_faceserpro', 'picture', $file->itemid);
            endforeach;
        }
    }
}
