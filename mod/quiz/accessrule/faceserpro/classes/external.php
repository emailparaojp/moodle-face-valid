...
/**
 * Extrarnal for the quizaccess_faceserpro plugin.
/**
 ...
 *
 * @package quizaccess_faceserpro
 * @Copyright (c) 2024 RCN
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizaccess_faceserpro_external extends external_api {
    /**
    ...
    public static function get_camshots($courseid, $quizid, $userid) {
        ...
        $warnings = array();
        if ($params['quizid']) {
            $camshots = $DB->get_records('quizaccess_faceserpro_logs', $params, 'id DESC');
        } else {
            $camshots = $DB->get_records('quizaccess_faceserpro_logs',
    ...
    public static function send_camshot
        ...
        if ($imagetype == 1) {
            ...
            $record->component = 'quizaccess_faceserpro';
            $record->filepath = '';
            ...
            $camshot = $DB->get_record('quizaccess_faceserpro_logs', array('id' => $screenshotid));
            $record = new stdClass();
            ...
            $screenshotid = $DB->insert_record('quizaccess_faceserpro_logs', $record, true);
            // Save the face image.
            $record = new stdClass();
            ...
            if ($faceimage) {
                ...
                $data = $faceimage;
                list(, $data) = explode(';', $data);
                $url = self::quizaccess_faceserpro_geturl_without_timecode(
            ...
            $record->timemodified = time();
            $screenshotid = $DB->insert_record('faceserpro_face_images', $record, true);
            $result = array();
    ...
    protected static function request_user_require_capability(array $params, context $context, $USER) {
        ...
        if ($USER->id != $user->id) {
            require_capability('quizaccess/faceserpro:viewreport', $context);
        }
    }
    /**
    ...
    private static function add_timecode_to_image ($data) {
        ...
        imagefttext($image, 9, 0, 4, 16, imagecolorallocate($image, 0, 0, 0),
            $CFG->dirroot . '/mod/quiz/accessrule/faceserpro/assets/Raleway-VariableFont_wght.ttf', date('d-m-Y H:i:s') );
        ob_start();
        imagepng($image);
        $data = ob_get_clean();
        ob_end_clean();
    ...
    public static function validate_face($courseid, $cmid, $profileimage, $webcampicture, $parenttype, $faceimage, $facefound) {
        ...
        $record->facefound = $facefound;
        $record->timemodified = time();
        $faceimageid = $DB->insert_record('faceserpro_face_images', $record, true);
        /*$profileimageurl = quizaccess_faceserpro_get_image_url( $USER->id);
        ...
        // Face check.
        require_once($CFG->dirroot.'/mod/quiz/accessrule/faceserpro/lib.php');
        $method = get_faceserpro_settings("fcmethod");
        if ($method == "AWS") {
            aws_analyze_specific_image($screenshotid);
        } else if ($method == "BS") {
            bs_analyze_specific_image_from_validate($screenshotid);
        }
        $currentdata = $DB->get_record('quizaccess_faceserpro_logs', array('id' => $screenshotid));
     ...
     * @return mixed
     */
    private static function quizaccess_faceserpro_geturl_without_timecode(
