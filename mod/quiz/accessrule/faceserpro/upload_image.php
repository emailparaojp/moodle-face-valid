...
/**
 * Upload image from users list in quizaccess_faceserpro plugin.
...
require_once($CFG->dirroot . '/mod/quiz/accessrule/faceserpro/classes/form/upload_image_form.php');
require_once($CFG->dirroot . '/mod/quiz/accessrule/faceserpro/lib.php');

$PAGE->set_url(new moodle_url('/mod/quiz/accessrule/faceserpro/upload_image.php'));
$PAGE->set_context(\context_system::instance());
$PAGE->set_title(get_string('upload_image_title', 'quizaccess_faceserpro'));
...
if ($mform->is_cancelled()) {
    redirect($CFG->wwwroot . '/mod/quiz/accessrule/faceserpro/userslist.php',
            get_string('cancel_image_upload', 'quizaccess_faceserpro'),
            null,
    ...
    $faceimagefile->component = 'quizaccess_faceserpro';
    ...
    $url = quizaccess_faceserpro_geturl_of_faceimage($faceimagedata, $userid, $faceimagefile, $context, $fs);
    ...
    if ($DB->record_exists_select('faceserpro_user_images', 'user_id = :id', array('id' => $data->id))) {
        $record = $DB->get_record_select('faceserpro_user_images', 'user_id = :id', array('id' => $data->id));
        $record->photo_draft_id = $data->user_photo;
        $DB->update_record('faceserpro_user_images', $record);

        // Save face image in face table.
        $facetablerecord->parentid = $record->id;

        if ($DB->record_exists('faceserpro_face_images',
                                array('parentid' => $facetablerecord->parentid,
                                    'parent_type' => $facetablerecord->parent_type))) {
            $facetablerow = $DB->get_record('faceserpro_face_images',
                                        array('parentid' => $facetablerecord->parentid,
                                            'parent_type' => $facetablerecord->parent_type));
            $facetablerecord->id = $facetablerow->id;
            $DB->update_record('faceserpro_face_images', $facetablerecord);
        } else {
            $DB->insert_record('faceserpro_face_images', $facetablerecord);
        }
        redirect($CFG->wwwroot . '/mod/quiz/accessrule/faceserpro/userslist.php',
                get_string('image_updated', 'quizaccess_faceserpro'),
                null,
        ...
        $record->photo_draft_id = $data->user_photo;
        $parentid = $DB->insert_record('faceserpro_user_images', $record);
...
$PAGE->requires->js_call_amd('quizaccess_faceserpro/validateAdminUploadedImage', 'setup', array($modelurl));
