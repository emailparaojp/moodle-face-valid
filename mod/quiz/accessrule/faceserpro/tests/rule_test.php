...
/**
 * Unit tests for the quizaccess_faceserpro plugin.
/**
...
require_once($CFG->dirroot.'/mod/quiz/accessrule/faceserpro/rule.php');
...
class rule_test extends advanced_testcase {
     ...
     */
    public function test_faceserpro_access_rule() {
        $quiz = new \stdClass();
        $cm = new \stdClass();
        $cm->id = 0;
        $quizobj = new \mod_quiz\quiz_settings($quiz, $cm, null);
        $rule = new \quizaccess_faceserpro($quizobj, 0);
    ...
    public function test_validate_preflight_check() {
        ...
        $string = get_string('youmustagree', 'quizaccess_faceserpro');
        $this->assertEquals($errors['faceserpro'], $string);
    }
    /*
     ...
     * @throws coding_exception
     */
    public function test_faceserpro_settings() {
        global $DB, $CFG;
        $this->resetAfterTest();
        $this->assertEquals('30', get_faceserpro_settings('autoreconfigurecamshotdelay'));
        $this->assertEquals('230', get_faceserpro_settings('autoreconfigureimagewidth'));
        $this->assertEquals('0', get_faceserpro_settings('awschecknumber'));
        $this->assertEquals('80', get_faceserpro_settings('awsfcthreshold'));
        $this->assertEquals('', get_faceserpro_settings('awskey'));
        $this->assertEquals('', get_faceserpro_settings('awssecret'));
        $this->assertEquals('', get_faceserpro_settings('bsapi'));
    }
    /*
     * Test save settings
     *
