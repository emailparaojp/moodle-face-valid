define("quizaccess_faceserpro/validateAdminUploadedImage", [
  "jquery",
  "core/ajax",
  "core/notification",
  ...
  const clearPreviousNotifications = () => {
    ...
    displayNotification = (message, type) => {
      Notification.addNotification({ message: message, type: type });
    },
    detectface = async (input, croppedImage) => {
  ...
  return {
    async setup(modelurl) {
      ...
      let previewImage,
        submitBtn = document.getElementById("id_submitbutton"),
      ...
      return (
        ...
        setInterval(async function () {
          ...
          if (preview.length > 0) {
            ...
            const splitArray = preview[0].src.split("?");
            if (previewImage.src === splitArray[0]) return;
            if (
              ...
              console.log("Face found"),
                ...
                0 == notificationShown &&
                  (displayNotification(
                    "Face found in the uploaded image",
                    "success"
                  ),
                  (notificationShown = 1));
              let faceImageField = document.querySelector(
                '[name="face_image"]'
              );
              faceImageField &&
              ...
              clearPreviousNotifications(),
                0 == notificationShown &&
                  (displayNotification(
                    "Face not found in the uploaded image",
                    "error"
                  ),
                  (notificationShown = 1)),
                (croppedImage.src = null),
                console.log("Face not found");
          } else submitBtn && (submitBtn.disabled = !0);
        }, 1e3),
        !0
      );
    },
