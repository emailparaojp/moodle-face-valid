...
/**
 * Library function for the quizaccess_faceserpro plugin.
...
/**
 * Generic select statement from faceserpro logs table.
 */
const COMMON_SELECT = ' e.timemodified as timemodified, u.firstname as firstname, u.lastname as lastname,
u.email as email from {quizaccess_faceserpro_logs} e INNER JOIN {user} u  ON u.id = e.userid ';
 ...
 * Query constants.
 */
const FROM_QUIZACCESS_faceserpro_LOGS_INNER_JOIN_USERS =
 ...
 */
function quizaccess_faceserpro_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    $file = $fs->get_file($context->id, 'quizaccess_faceserpro', $filearea, $itemid, $filepath, $filename);
    if (!$file) {
        return false;
    }
 ...
 */
function quizaccess_faceserpro_get_image_url($userid) {
    $context = context_system::instance();

    $fs = get_file_storage();
    if ($files = $fs->get_area_files($context->id, 'quizaccess_faceserpro', 'user_photo')) {
 ...
 */
function quizaccess_faceserpro_get_image_file($userid)
{
    global $DB;
    $context = context_system::instance();
    ...
    if ($files = $fs->get_area_files($context->id, 'quizaccess_faceserpro', 'user_photo')) {
        ...
        foreach ($files as $file) {
            if ($userid == $file->get_itemid() && $file->get_filename() != '.') {
                ...
                $record_id = $DB->get_field('faceserpro_user_images', 'id', array('user_id' => $userid));

                // Delete the record from the database
                $DB->delete_records('faceserpro_user_images', array('user_id' => $userid));

                // Delete associated row from faceserpro_face_images table
                $DB->delete_records('faceserpro_face_images', array('parentid' => $record_id));
...
function update_match_result($rowid, $matchresult, $awsflag) {
    ...
    $updatesql = "UPDATE {quizaccess_faceserpro_logs} SET awsflag = '$awsflag', awsscore = '$score' WHERE id='$rowid'";
...
function check_similarity_aws($referenceimageurl, $targetimageurl) {
    ...
    $awskey = get_faceserpro_settings('awskey');
    $awssecret = get_faceserpro_settings('awssecret');
    $threshhold = (int)get_faceserpro_settings('awsfcthreshold');
...
function execute_fm_task() {
    global $DB;
    // Get 5 task.
    $sql = 'SELECT * FROM {faceserpro_facematch_task} LIMIT 5';
    $data = $DB->get_recordset_sql($sql);
    $facematchmethod = get_faceserpro_settings('fcmethod');
    foreach ($data as $row) {
        $rowid = $row->id;
        ...
        if ($facematchmethod == 'AWS') {
            ...
            $DB->delete_records('faceserpro_facematch_task', ['id' => $rowid]);
...
function log_facematch_task() {
    global $DB;
    $sql = 'SELECT DISTINCT courseid,quizid,userid FROM {quizaccess_faceserpro_logs}  WHERE awsflag = 0';
    $data = $DB->get_recordset_sql($sql);
    foreach ($data as $row) {
...
function log_specific_quiz($courseid, $cmid, $studentid) {
    ...
    $updatesql = 'UPDATE {quizaccess_faceserpro_logs}'
        . 'SET awsflag = 1'
    ...
    // Check random.
    $limit = 5;
    $awschecknumber = get_faceserpro_settings('awschecknumber');
    if ($awschecknumber != '') {
    ...
    if ($limit == -1) {
        ...
        $sql = GENERIC_SELECT_STATMENT
            ...
            . FROM_QUIZACCESS_faceserpro_LOGS_INNER_JOIN_USERS
            . " WHERE e.courseid = '$courseid' AND e.quizid = '$cmid' AND u.id = '$studentid' AND e.webcampicture != '' "
            . ' ORDER BY RAND() '
            . " LIMIT $limit ";
    } else {
    ...
    foreach ($sqlexecuted as $row) {
        ...
        if ($snapshot != '') {
            ...
            $DB->insert_record('faceserpro_facematch_task', $inserttaskrow);
        }
    }
...
/**
 * Get faceserpro settings values.
 *
 * @param string $settingtype the courseid
 *
 * @return string
 */
function get_faceserpro_settings($settingtype) {
    $value = '';
    global $DB;
    $settingssql = "SELECT * FROM {config_plugins}
            WHERE {config_plugins}.plugin = 'quizaccess_faceserpro' AND {config_plugins}.name = '$settingtype'";
...
function aws_analyze_specific_image($reportid) {
    global $DB;
    $reportsql = 'SELECT id,courseid,quizid,userid,webcampicture FROM {quizaccess_faceserpro_logs} WHERE id=:id';
...
function get_face_images($reportid) {
    global $DB;
    $reportdata = $DB->get_record('quizaccess_faceserpro_logs', array('id' => $reportid));
    $studentid = $reportdata->userid;
    $webcamfaceimage = $DB->get_record('faceserpro_face_images', array('parentid' => $reportid, 'parent_type' => 'camshot_image'));
    $webcamfaceimageurl = "";
    if ($webcamfaceimage) {
        $webcamfaceimageurl = $webcamfaceimage->faceimage;
    }
    $userimagerow = $DB->get_record('faceserpro_user_images', array('user_id' => $studentid));
...
function extracted($profileimageurl, $targetimage, int $reportid): void {
    ...
    $threshold = get_faceserpro_settings('threshold');
    // Update Match result.
...
function check_similarity_bs($referenceimageurl, $targetimageurl) {
    global $CFG;
    $bsapi = get_faceserpro_settings('bsapi');
    $threshold = get_faceserpro_settings('threshold');

    $bsapikey = get_faceserpro_settings('bs_api_key');
    // Load File.
...
function get_token() {
    ...
    $bsapi = get_faceserpro_settings('bsapi') . '/get_token';
    $bsusername = get_faceserpro_settings('username');
    $bspassword = get_faceserpro_settings('password');
    // Check similarity.
    $curl = curl_init();
    curl_setopt_array($curl, [
        CURLOPT_URL => $bsapi,
...
function log_fm_warning($reportid) {
    global $DB;
    $reportsql = 'SELECT * FROM {quizaccess_faceserpro_logs} WHERE id=:id';
    $reportdata = $DB->get_record_sql($reportsql, ['id' => $reportid]);
    ...
    if ($reportdata) {
        ...
        $warningsql = 'SELECT * FROM {faceserpro_fm_warnings} WHERE userid = :userid AND courseid = :courseid AND quizid = :quizid';
        ...
        if (!$warnings) {
            ...
            $DB->insert_record('faceserpro_fm_warnings', $warning);
        }
    }
}
 ...
 * @return mixed
 */
function quizaccess_faceserpro_geturl_of_faceimage(string $data, int $userid, stdClass $record, $context, $fs) {
